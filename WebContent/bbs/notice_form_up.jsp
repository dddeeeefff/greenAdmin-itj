...
BbsNotice noticeUpInfo = (BbsNotice)request.getAttribute("noticeUpInfo");
%>
<style>
#page-contents {
...
<div id="page-contents">
	...
	<div class="frm_frame">
		<form name="frm" enctype="multipart/form-data">
			<div><input type="text" name="addTitle" class="addTitle" id = "addTitle" value="<%=noticeUpInfo.getBn_title() %>" placeholder="제목을 입력해주세요." /></div><br />
			<div><input type="file" name="addFile" class="addFile" id = "addFile" value="<%=noticeUpInfo.getBn_img() %>" accept="notice_img/*" onchange="validation(this);" /></div><br />
			<div>
				<input type="hidden" name="addContent" id="addContent" />
				<textarea name="writeContent" class='writeContent' id="writeContent"placeholder="내용을 입력해주세요."><%=noticeUpInfo.getBn_content() %></textarea>
			</div>
			<br />
			<div class="btn_box">
				<input class="cancel" type="button" value="취소" onclick="history.back();" />&nbsp;
				<input class="regist" type="button" onclick="notice_submit();" value="수정" />
				<input type="hidden" name="bnidx" value="<%=noticeUpInfo.getBn_idx() %>" />
			</div>
		</form>
	</div>
</div>
