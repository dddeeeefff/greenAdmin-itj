...
#sdate, #edate { width:130px; height:30px; padding:5px; }
...
<div id="page-contents">
	...
	<div class="frm_frame">
		<form name="frmEvent" enctype="multipart/form-data">
			...
			<div><input type="text" name="be_title" id="be_title" placeholder="제목을 입력해 주세요." value="<%=be.getBe_title() %>" /></div><br />
			<div><input type="file" name="be_img" id="be_img" accept="image/*" onchange="validation(this);" /></div><br />
			<div>
				이벤트 기간 : 시작일&nbsp;&nbsp;
				<input type="date" name="sdate" id="sdate" min="<%=today %>" value="<%=be.getBe_sdate() %>" />&nbsp;&nbsp;~&nbsp;
				종료일&nbsp;&nbsp;
				<input type="date" name="edate" id="edate" min="<%=today %>" value="<%=be.getBe_edate() %>" />
			</div><br/>	
			<textarea name="be_content" id="be_content" placeholder="내용을 입력해주세요."><%=be.getBe_content() %></textarea><br/><br/>
			<div id="btn_box">
				<input type="button" id="cancel" value="취소" onclick="history.back();" />&nbsp;
				<input type="button" id="btnsch" value="등록" onclick="eventProcUp();" />
			</div>
		</form>
	</div>
</div>
